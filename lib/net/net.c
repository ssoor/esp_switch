...
int net_write_with_timeout(net_conn_t conn, ulong usec, const void *data, uint32 lenght)
{
    ...
    {
        //设置发送超时
        setsockopt(_conn->fd, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout, sizeof(struct timeval));
    }
...
int net_read_with_timeout(net_conn_t conn, ulong usec, void *data, uint32 lenght)
{
    ...
    {
        //设置接收超时
        setsockopt(_conn->fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout, sizeof(struct timeval));
    }
